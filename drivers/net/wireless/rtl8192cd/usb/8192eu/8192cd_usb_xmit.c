...
inline void update_remaining_timeslot(struct rtl8192cd_priv *priv, struct tx_desc_info *pdescinfo, unsigned int q_num, unsigned char is_40m_bw)
{
	...
	ts_consumed = ((pdescinfo->buf_len[0])*16)/(txRate); // unit: microsecond (us)
...
void rtl8192cd_hal_fill_fwinfo(struct rtl8192cd_priv *priv, struct tx_insn* txcfg, struct tx_desc  *pdesc, unsigned int frag_idx)
{
	...
	if (txcfg->need_ack) { // unicast
		if (frag_idx == 0) 
			RtsCheck(priv, txcfg, txcfg->fr_len,
				&bRtsEnable, &bCts2SelfEnable, &bHwRts, &bErpProtect, &bNProtect);
	}
...
//			&& !GetMData(txcfg->phdr)
		...
		ptxsc_entry = &txcfg->pstat->tx_sc_ent_usb[pktpri][idx];
...
void signin_beacon_desc_usb(struct rtl8192cd_priv *priv, unsigned int *beaconbuf, unsigned int frlen)
{
	struct tx_desc *pdesc;
	struct tx_desc_info	*pdescinfo;
	struct xmit_buf *pxmitbuf;
...
#if 1 // ZYXEL debug.
		/* Do not print massive message for a long period time when 2.4G in some unknown circumstance. */
#else
		if (net_ratelimit())
