...
/* Ugly declaration for pid. */
static u32 zyxel_usr_pid, msg_dropcounter;
/* Function decorateion*/
void zyxel_netlink_rcv(struct sk_buff *skb);
void construct_netlink_msg(void*);
void zyxel_netlink_sendmsg(int pid, struct sock *nl_sock, void *payload, int payload_len);
...
//#endif
...
#if defined(CONFIG_ZYXEL_KERNEL_EVENT_LOG)
	/* Get original wifi channel */
	int orig_channel = priv->pmib->dot11RFEntry.dot11channel;
#endif // CONFIG_ZYXEL_KERNEL_EVENT_LOG
	...
	/* dfs event detected, send msg. */
	kern_event_t event_data;
	event_data.msgtype = e_dfs_event;
	dfs_event_p dfs_data = (dfs_event_p)&event_data.comm_data;
	dfs_data->orig_ch = orig_channel;
	dfs_data->new_ch = priv->pmib->dot11RFEntry.dot11channel;
	dfs_data->bw = (priv->pshare->CurrentChannelBW == 2)?80: ((priv->pshare->CurrentChannelBW == 1)?40:20);
	construct_netlink_msg((void*)&event_data);
#endif // CONFIG_ZYXEL_KERNEL_EVENT_LOG
	SAVE_INT_AND_CLI(flags);
	SMP_LOCK(flags);
...
#if defined(CONFIG_ZYXEL_KERNEL_EVENT_LOG)
int zyxel_netlink_init(void) {

#if defined(__LINUX_3_10__)
	struct netlink_kernel_cfg cfg = {
		.input = zyxel_netlink_rcv,
	};

	nl_sock = netlink_kernel_create(&init_net, NETLINK_ZYXEL_CUSTOMIZE_KERNEL_EVENT, &cfg);
#else
	nl_sock = netlink_kernel_create(&init_net, NETLINK_ZYXEL_CUSTOMIZE_KERNEL_EVENT, 0, zyxel_netlink_rcv, NULL, THIS_MODULE);
#endif

	if (!nl_sock) {
		panic_printk(KERN_ERR "nl_sock: Cannot create netlink socket");
		return -ENOMEM;
	}
...
void zyxel_netlink_rcv(struct sk_buff *skb) {
	...
	void *msg;
	kern_event_t recv_data;
	memset(&recv_data, 0, sizeof(kern_event_t));
	...
	panic_printk(KERN_INFO "kernel received msg payload:%s\n",(unsigned char*)NLMSG_DATA(nlh));

	msg = NLMSG_DATA(nlh);
	zyxel_usr_pid = nlh->nlmsg_pid; /* pid of user space process. */
}

void construct_netlink_msg(void* pData) {

	void *payload = NULL;
	int payload_len = 0;
	kern_event_p pSend = (kern_event_p)pData;
	unsigned char type = pSend->msgtype;
	payload_len = sizeof(kern_event_t);
	payload = pData;

	zyxel_netlink_sendmsg(zyxel_usr_pid, nl_sock, payload, payload_len);
}
...
void zyxel_netlink_sendmsg(int pid, struct sock *nl_sock, void *payload, int payload_len) {
	...
	skblen = NLMSG_SPACE(payload_len);
	...
	memset(datab, 0, payload_len);
	memcpy(datab, payload, payload_len);

	nlh->nlmsg_len = NLMSG_HDRLEN + payload_len;

	err = netlink_unicast(nl_sock, skb, zyxel_usr_pid, MSG_DONTWAIT);
...
EXPORT_SYMBOL(construct_netlink_msg);
#endif // CONFIG_ZYXEL_KERNEL_EVENT_LOG
