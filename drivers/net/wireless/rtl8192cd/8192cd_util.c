...
#if defined(CONFIG_PCI_HCI)
	if (priv->hci_type == RTL_HCI_PCIE) {
		free_sta_tx_skb_pci(priv, pstat);
	}
#endif
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	if (priv->hci_type == RTL_HCI_USB || priv->hci_type == RTL_HCI_SDIO) {
		free_sta_tx_skb_usb(priv, pstat);
	}
#endif
}
#endif
...
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	if (priv->hci_type == RTL_HCI_USB || priv->hci_type == RTL_HCI_SDIO) {
		memset(pstat->tx_sc_ent_usb, 0, sizeof(pstat->tx_sc_ent_usb));
	}
#endif
}
#endif
...
/* return 0 if found corresponding mesh node
 *        -ENOENT if failed
 */
int		batadv_get_orig_info
 ...
 struct wifi_mesh_node *node)
{
	...
	pstat=get_stainfo(priv, node->orig);
	if(pstat) {
		retval=0;
		/*printk(KERN_INFO "[%s] rssi:%d cu:%d noise:%d\n", 
			netdev->name, pstat->mesh_neighbor_TBL.rssi_metric, 
			pstat->mesh_neighbor_TBL.cu_metric, 
			pstat->mesh_neighbor_TBL.noise_metric);*/
		node->rssi_metric=pstat->mesh_neighbor_TBL.rssi_metric;
		node->cu_metric=pstat->mesh_neighbor_TBL.cu_metric;
		node->noise_metric=pstat->mesh_neighbor_TBL.noise_metric;
	}
	return	retval;
}
#endif	//BATMAN_ENABLE
...
static struct channel_list reg_channel_5g_full_band[] = {
	...
	/* ZYXEL FCC1 */{{100,104,108,112,116,120,124,128,136,140,149,153,157,161,165},15},
	/* IC */		{{36,40,44,48,52,56,60,64,149,153,157,161},12},
	/* ETSI */		//{{36,40,44,48,52,56,60,64,100,104,108,112,116,120,124,128,132,136,140},19},
	/* ZYXEL ETSI 1 */ {{100,104,108,112,116,120,124,128,132,136,140},11},
	...
	if (priv->hci_type == RTL_HCI_USB || priv->hci_type == RTL_HCI_SDIO) {	
		...
		for (i = 0; i < NR_NET80211_UP; ++i) {
			for (j = 0; j < TX_SC_ENTRY_NUM; ++j)
				pstat->tx_sc_ent_usb[i][j].hwdesc1.Dword7 &= set_desc(~TX_TxBufSizeMask);
		}
	}
#endif
#if defined(CONFIG_PCI_HCI)
...
#if defined(DFS_METEOROLOGY_REFINE) && (defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_WLAN_HAL))

void update_AC_STA_RAMask(struct rtl8192cd_priv *priv)
{
	int idx = 0;
	struct stat_info *pstat = NULL;
	if (!IS_HAL_CHIP(priv) && (GET_CHIP_VER(priv) != VERSION_8812E))
	...
	while (pstat) {
		if(pstat->vht_cap_len) {
			...
			if(priv->pmib->dot11BssType.net_work_type & WIRELESS_11AC) 
				pstat->tx_bw = priv->pmib->dot11nConfigEntry.dot11nUse40M;
#endif
