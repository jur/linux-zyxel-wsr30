...
#if defined(TRIBAND_MESH_SINGLE_IFACE)
...
struct _device_info_ wlan_device[] =
{
	...
	#if defined(CONFIG_SLOT_0_8822BE)//SLOT_0 
		{
			(MESH_NUM_CFG<<MESH_SHIFT) |(WDS_NUM_CFG<<WDS_SHIFT) | (TYPE_PCI_DIRECT<<TYPE_SHIFT) | ACCESS_SWAP_MEM, 
			BSP_PCIE0_D_CFG0, 
			BSP_PCIE0_D_MEM, 
	...
	#if defined(CONFIG_SLOT_0_8822BE) //5G+5G+2G
		{
			(MESH_NUM_CFG<<MESH_SHIFT) | (WDS_NUM_CFG<<WDS_SHIFT) | (TYPE_PCI_BIOS<<TYPE_SHIFT) | ACCESS_SWAP_MEM, 
			0, 
			((0x8191<<16)|USB_VENDER_ID_REALTEK), 
			0, 
			NULL
		},
	#else //5G+2G
...
static int rtw_shutdown(struct notifier_block *nb, unsigned long cmd, void *p)
{
	...
	if (wlan_device[USB_DEVICE_IDX()].priv) {
		dev = dev_get_by_name(&init_net, wlan_device[USB_DEVICE_IDX()].priv->dev->name);
		if (dev) {
			printk("[%s] close %s ...\n", __func__, wlan_device[USB_DEVICE_IDX()].priv->dev->name);
			rtnl_lock();
			dev_close(dev);
			rtnl_unlock();
...
static int rtl8192cd_init_sw(struct rtl8192cd_priv *priv)
{
...
#endif
	{
		...
		priv->pmib->dot11DFSEntry.CAC_ss_counter = 0;
		priv->pshare->rf_ft_var.dfs_original_bw = priv->pmib->dot11nConfigEntry.dot11nUse40M;
	}
#endif
...
#endif
	{
		...
		{
#ifdef CHECK_HANGUP
			if (priv->hci_type == RTL_HCI_USB || (priv->hci_type == RTL_HCI_PCIE && !priv->reset_hangup))
#endif
			{
...
//------------------------- david+2007-5-31
    ...
    // if vap enabled, set beacon int to 200 at minimun when guest ssid num > 4
	{
		...
		if ((OPMODE & WIFI_AP_STATE) && GET_ROOT(priv)->pmib->miscEntry.vap_enable)
		{
			...
			if (ssid_num >= (priv->hci_type == RTL_HCI_PCIE ? 5:4))
				minbcn_period = 200;
			else
				minbcn_period = 100;
...
static int rtl8192cd_stop_sw(struct rtl8192cd_priv *priv)
{
...
#endif
	{
		...
		if (priv->pmib->wapiInfo.wapiType!=wapiDisable
#ifdef CHECK_HANGUP
			&& (priv->hci_type == RTL_HCI_USB || (priv->hci_type == RTL_HCI_PCIE && !priv->reset_hangup))
#endif
#ifdef SMART_REPEATER_MODE
	...
	if( priv->pmib->miscEntry.vap_enable) {
	  for (i=0; i<RTL8192CD_NUM_VWLAN; i++){
		  if (priv->pvap_priv[i]->pmib->wapiInfo.wapiType!=wapiDisable
#ifdef CHECK_HANGUP
			&& (priv->hci_type == RTL_HCI_USB || (priv->hci_type == RTL_HCI_PCIE && !priv->pvap_priv[i]->reset_hangup))
#endif
			) {
...
static unsigned char get_lmt_pwr_AC(struct rtl8192cd_priv *priv, unsigned int tx_rate, unsigned char channel, unsigned char bw)
{
	unsigned char working_channel = 0;
	unsigned char txpwr_lmt;
	int idx;

	if (bw == 2) {
		working_channel = get_center_channel(priv, channel, priv->pshare->offset_2nd_chan, 1);
	} else if (bw == 1) {
		working_channel = (priv->pshare->offset_2nd_chan == 1) ? (channel - 2) : (channel + 2);
	} else if (bw == 0) {
		working_channel = channel;
	}

	idx = ch2idx(working_channel);

#if defined(CONFIG_WLAN_HAL_8814AE)
	if (((tx_rate - VHT_RATE_ID) / 10) + 1 == 4) {			// VHT NSS 4
		switch (bw)
		{
		case 0: txpwr_lmt = priv->pshare->ch_pwr_lmtHT20_4S[idx]; break;
		case 1: txpwr_lmt = priv->pshare->ch_pwr_lmtHT40_4S[idx]; break;
		case 2: txpwr_lmt = priv->pshare->ch_pwr_lmtVHT80_4S[idx]; break;
		}
	}
	else if (((tx_rate - VHT_RATE_ID) / 10) + 1 == 3) {		// VHT NSS 3
		switch (bw)
		{
		case 0: txpwr_lmt = priv->pshare->ch_pwr_lmtHT20_3S[idx]; break;
		case 1: txpwr_lmt = priv->pshare->ch_pwr_lmtHT40_3S[idx]; break;
		case 2: txpwr_lmt = priv->pshare->ch_pwr_lmtVHT80_3S[idx]; break;
		}
	}
	else 
#endif
	if (((tx_rate - VHT_RATE_ID) / 10) + 1 == 2) {			// VHT NSS 2
		switch (bw)
		{
		case 0: txpwr_lmt = priv->pshare->ch_pwr_lmtHT20_2S[idx]; break;
		case 1: txpwr_lmt = priv->pshare->ch_pwr_lmtHT40_2S[idx]; break;
		case 2: txpwr_lmt = priv->pshare->ch_pwr_lmtVHT80_2S[idx]; break;
		}
	}
	else if (((tx_rate - VHT_RATE_ID) / 10) + 1 == 1)	 {	// VHT NSS 1
		switch (bw)
		{
		case 0: txpwr_lmt = priv->pshare->ch_pwr_lmtHT20_1S[idx]; break;
		case 1: txpwr_lmt = priv->pshare->ch_pwr_lmtHT40_1S[idx]; break;
		case 2: txpwr_lmt = priv->pshare->ch_pwr_lmtVHT80_1S[idx]; break;
		}
	}
#if defined(CONFIG_WLAN_HAL_8814AE)
	else if (tx_rate - HT_RATE_ID > 23) {
		switch (bw)
		{
		case 0: txpwr_lmt = priv->pshare->ch_pwr_lmtHT20_4S[idx]; break;
		case 1: txpwr_lmt = priv->pshare->ch_pwr_lmtHT40_4S[idx]; break;
		case 2: txpwr_lmt = priv->pshare->ch_pwr_lmtHT40_4S[idx]; break;
		}
	}
	else if (tx_rate - HT_RATE_ID > 15) {
		switch (bw)
		{
		case 0: txpwr_lmt = priv->pshare->ch_pwr_lmtHT20_3S[idx]; break;
		case 1: txpwr_lmt = priv->pshare->ch_pwr_lmtHT40_3S[idx]; break;
		case 2: txpwr_lmt = priv->pshare->ch_pwr_lmtHT40_3S[idx]; break;
		}
	}
#endif
	else if (tx_rate - HT_RATE_ID > 7) {
		switch (bw)
		{
		case 0: txpwr_lmt = priv->pshare->ch_pwr_lmtHT20_2S[idx]; break;
		case 1: txpwr_lmt = priv->pshare->ch_pwr_lmtHT40_2S[idx]; break;
		case 2: txpwr_lmt = priv->pshare->ch_pwr_lmtHT40_2S[idx]; break;
		}
	}
	else if (tx_rate - HT_RATE_ID >= 0) {
		switch (bw)
		{
		case 0: txpwr_lmt = priv->pshare->ch_pwr_lmtHT20_1S[idx]; break;
		case 1: txpwr_lmt = priv->pshare->ch_pwr_lmtHT40_1S[idx]; break;
		case 2: txpwr_lmt = priv->pshare->ch_pwr_lmtHT40_1S[idx]; break;
		}
	}
	else {	// OFDM
		txpwr_lmt = priv->pshare->ch_pwr_lmtOFDM[idx];
	}

	return txpwr_lmt;
}

/* return target power of highest rate 
 */
static unsigned char get_tg_pwr_AC(struct rtl8192cd_priv *priv, unsigned int tx_rate, unsigned char path)
{
	unsigned char tg_pwr = 0;

	if (priv->pmib->dot11RFEntry.MIMO_TR_mode == MIMO_2T2R) {
		if (tx_rate < HT_RATE_ID)				// OFDM
			tg_pwr = priv->pshare->tgpwr_OFDM_new[path];
		else if (tx_rate - HT_RATE_ID <= 7)		// HT 1SS
			tg_pwr = priv->pshare->tgpwr_HT1S_new[path];
		else if (tx_rate - HT_RATE_ID <= 15)	// HT 2SS
			tg_pwr = priv->pshare->tgpwr_HT2S_new[path];
		else if (((tx_rate - VHT_RATE_ID) / 10) + 1 == 1)	// VHT 1SS
			tg_pwr = priv->pshare->tgpwr_VHT1S_new[path];
		else if (((tx_rate - VHT_RATE_ID) / 10) + 1 == 2)	// VHT 2SS
			tg_pwr = priv->pshare->tgpwr_VHT2S_new[path];
	}

	return tg_pwr;
}

static unsigned char get_tx_pwr_AC(struct rtl8192cd_priv *priv, unsigned int tx_rate, unsigned char channel, unsigned char bw, unsigned char path)
{
	unsigned char ch_idx, ch_idx_ht40, ch_idx_vht80;
	unsigned char txpwr, tpi = 0;
	char diff_1s = 0, diff_2s = 0;
	unsigned int i;

	ch_idx = channel - 1;
	ch_idx_ht40 = ((priv->pshare->offset_2nd_chan == 1) ? (channel - 2) : (channel + 2)) - 1;
	ch_idx_vht80 = get_center_channel(priv, channel, priv->pshare->offset_2nd_chan, 1);
	...
	if (priv->pmib->dot11RFEntry.MIMO_TR_mode == MIMO_2T2R) {
		if (tx_rate < HT_RATE_ID) {		// OFDM
			...
			{
			case 0:
				txpwr = priv->pmib->dot11RFEntry.pwrlevel5GHT40_1S_A[ch_idx];
				diff_1s = (priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_A[ch_idx] & 0x0f);
				break;
			case 1:
				txpwr = priv->pmib->dot11RFEntry.pwrlevel5GHT40_1S_B[ch_idx];
				diff_1s = (priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_B[ch_idx] & 0x0f);
				break;
			}
			diff_1s = convert_diff_88XX_AC(diff_1s);
			tpi = txpwr + diff_1s;
		}
		...
		if (bw == 0) {		// 20M
			switch (path)
			{
			case 0: txpwr = priv->pmib->dot11RFEntry.pwrlevel5GHT40_1S_A[ch_idx]; break;
			case 1: txpwr = priv->pmib->dot11RFEntry.pwrlevel5GHT40_1S_B[ch_idx]; break;
			}
		
			if (tx_rate - HT_RATE_ID <= 7
				|| ((tx_rate - VHT_RATE_ID) / 10) + 1 == 1) {		// 1SS
				switch (path)
				{
				case 0:
					diff_1s = ((priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_A[ch_idx] & 0xf0) >> 4); break;
				case 1:
					diff_1s = ((priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_B[ch_idx] & 0xf0) >> 4); break;
				}
				diff_1s = convert_diff_88XX_AC(diff_1s);
				tpi = txpwr + diff_1s;
			}
			else if (tx_rate - HT_RATE_ID <= 15
				|| ((tx_rate - VHT_RATE_ID) / 10) + 1 == 2) {		// 2SS
				switch (path)
				{
				case 0:
					diff_1s = ((priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_A[ch_idx] & 0xf0) >> 4);
					diff_2s = (priv->pmib->dot11RFEntry.pwrdiff_5G_40BW2S_20BW2S_A[ch_idx] & 0x0f);
					break;
				case 1:
					diff_1s = ((priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_B[ch_idx] & 0xf0) >> 4);
					diff_2s = (priv->pmib->dot11RFEntry.pwrdiff_5G_40BW2S_20BW2S_B[ch_idx] & 0x0f);
					break;
				}
				diff_1s = convert_diff_88XX_AC(diff_1s);
				diff_2s = convert_diff_88XX_AC(diff_2s);
				tpi = txpwr + diff_1s + diff_2s;
			}
		}
		else if (bw == 1) {		// 40M
			switch (path)
			{
			case 0: txpwr = priv->pmib->dot11RFEntry.pwrlevel5GHT40_1S_A[ch_idx_ht40]; break;
			case 1: txpwr = priv->pmib->dot11RFEntry.pwrlevel5GHT40_1S_B[ch_idx_ht40]; break;
			}
			...
			if (tx_rate - HT_RATE_ID <= 7
				...
				{
				case 0:
					diff_2s = ((priv->pmib->dot11RFEntry.pwrdiff_5G_40BW2S_20BW2S_A[ch_idx] & 0xf0) >> 4); break;
				case 1:
					diff_2s = ((priv->pmib->dot11RFEntry.pwrdiff_5G_40BW2S_20BW2S_B[ch_idx] & 0xf0) >> 4); break;
				}
				diff_2s = convert_diff_88XX_AC(diff_2s);
				tpi = txpwr + diff_2s;
			}
			...
			{
			case 0:
				txpwr = (priv->pmib->dot11RFEntry.pwrlevel5GHT40_1S_A[ch_idx_vht80 - 4] +
						 priv->pmib->dot11RFEntry.pwrlevel5GHT40_1S_A[ch_idx_vht80 + 4]) / 2;
				break;
			case 1:
				txpwr = (priv->pmib->dot11RFEntry.pwrlevel5GHT40_1S_B[ch_idx_vht80 - 4] +
			...
			if (tx_rate - HT_RATE_ID <= 7
				...
				{
				case 0:
					diff_1s = ((priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_A[ch_idx] & 0xf0) >> 4); break;
				case 1:
					diff_1s = ((priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_B[ch_idx] & 0xf0) >> 4); break;
				}
				diff_1s = convert_diff_88XX_AC(diff_1s);
				tpi = txpwr + diff_1s;
			}
				...
				{
				case 0:
					diff_1s = ((priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_A[ch_idx] & 0xf0) >> 4);
					diff_2s = ((priv->pmib->dot11RFEntry.pwrdiff_5G_80BW2S_160BW2S_A[ch_idx] & 0xf0) >> 4);
					break;
				case 1:
					diff_1s = ((priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_B[ch_idx] & 0xf0) >> 4);
					diff_2s = ((priv->pmib->dot11RFEntry.pwrdiff_5G_80BW2S_160BW2S_B[ch_idx] & 0xf0) >> 4);
					break;
				}
				diff_1s = convert_diff_88XX_AC(diff_1s);
				diff_2s = convert_diff_88XX_AC(diff_2s);
...
static char calculate_txpwr_offset_AC(struct rtl8192cd_priv *priv, unsigned char tx_rate, unsigned char tx_bw)
{
	PDM_ODM_T odm_ptr = &(priv->pshare->_dmODM);
	unsigned char channel, curr_bw;
	unsigned int i;
	
	unsigned char curr_pwrlmt, new_pwrlmt, tg_pwr[2], curr_max_tpi[2], new_max_tpi[2];
	unsigned char curr_txpwr[2], new_txpwr[2], curr_high_rate_txpwr[2], new_high_rate_txpwr[2];
	unsigned char curr_txpwr_min[2], new_txpwr_min[2];
	char pwr_offset = 0;

	channel = priv->pmib->dot11RFEntry.dot11channel;
	curr_bw = priv->pshare->CurrentChannelBW;

	if (priv->pmib->dot11RFEntry.MIMO_TR_mode == MIMO_2T2R) {
		curr_high_rate_txpwr[RF_PATH_A] = get_tx_pwr_AC(priv, tx_rate, channel, curr_bw, RF_PATH_A);
		curr_high_rate_txpwr[RF_PATH_B] = get_tx_pwr_AC(priv, tx_rate, channel, curr_bw, RF_PATH_B);
		new_high_rate_txpwr[RF_PATH_A] = get_tx_pwr_AC(priv, tx_rate, channel, tx_bw, RF_PATH_A);
		new_high_rate_txpwr[RF_PATH_B] = get_tx_pwr_AC(priv, tx_rate, channel, tx_bw, RF_PATH_B);

		// set power by rate		
		if (priv->pmib->dot11RFEntry.MIMO_TR_mode == MIMO_2T2R) {
			i = tx_rate - VHT_RATE_ID;

			if(tx_rate == _NSS1_MCS8_RATE_ || tx_rate == _NSS1_MCS9_RATE_
			|| tx_rate == _NSS2_MCS8_RATE_ || tx_rate == _NSS2_MCS9_RATE_) {
				curr_txpwr[RF_PATH_A] = curr_high_rate_txpwr[RF_PATH_A] - priv->pshare->phw->VHTTxAgcOffset_A[i];
				curr_txpwr[RF_PATH_B] = curr_high_rate_txpwr[RF_PATH_B] - priv->pshare->phw->VHTTxAgcOffset_B[i];
				new_txpwr[RF_PATH_A] = new_high_rate_txpwr[RF_PATH_A] - priv->pshare->phw->VHTTxAgcOffset_A[i];
				new_txpwr[RF_PATH_B] = new_high_rate_txpwr[RF_PATH_B] - priv->pshare->phw->VHTTxAgcOffset_B[i];
			} else {
				curr_txpwr[RF_PATH_A] = curr_high_rate_txpwr[RF_PATH_A] + priv->pshare->phw->VHTTxAgcOffset_A[i];
				curr_txpwr[RF_PATH_B] = curr_high_rate_txpwr[RF_PATH_B] + priv->pshare->phw->VHTTxAgcOffset_B[i];
				new_txpwr[RF_PATH_A] = new_high_rate_txpwr[RF_PATH_A] + priv->pshare->phw->VHTTxAgcOffset_A[i];
		...
		if (!priv->pshare->rf_ft_var.disable_txpwrlmt) {
			tg_pwr[RF_PATH_A] = get_tg_pwr_AC(priv, tx_rate, RF_PATH_A);
			tg_pwr[RF_PATH_B] = get_tg_pwr_AC(priv, tx_rate, RF_PATH_B);
		
			curr_pwrlmt = get_lmt_pwr_AC(priv, tx_rate, channel, curr_bw);
			new_pwrlmt = get_lmt_pwr_AC(priv, tx_rate, channel, tx_bw);

			curr_max_tpi[RF_PATH_A] = curr_high_rate_txpwr[RF_PATH_A] + (curr_pwrlmt - tg_pwr[RF_PATH_A]);
			curr_max_tpi[RF_PATH_B] = curr_high_rate_txpwr[RF_PATH_B] + (curr_pwrlmt - tg_pwr[RF_PATH_B]);
			new_max_tpi[RF_PATH_A] = new_high_rate_txpwr[RF_PATH_A] + (new_pwrlmt - tg_pwr[RF_PATH_A]);
			new_max_tpi[RF_PATH_B] = new_high_rate_txpwr[RF_PATH_B] + (new_pwrlmt - tg_pwr[RF_PATH_B]);
			
			curr_txpwr_min[RF_PATH_A] = min(curr_txpwr[RF_PATH_A], curr_max_tpi[RF_PATH_A]);
			curr_txpwr_min[RF_PATH_B] = min(curr_txpwr[RF_PATH_B], curr_max_tpi[RF_PATH_B]);
			new_txpwr_min[RF_PATH_A] = min(new_txpwr[RF_PATH_A], new_max_tpi[RF_PATH_A]);
			new_txpwr_min[RF_PATH_B] = min(new_txpwr[RF_PATH_B], new_max_tpi[RF_PATH_B]);
		}

		pwr_offset = min(new_txpwr_min[RF_PATH_A] - curr_txpwr_min[RF_PATH_A], 
						 new_txpwr_min[RF_PATH_B] - curr_txpwr_min[RF_PATH_B]);
		pwr_offset = pwr_offset >> 1;	// tpi to dbm
	}

	// In some case, shrink may use low tx power, so tx power offset will be negative
	if (pwr_offset < 0)
		pwr_offset = 0;

#if 0	// debug print
	panic_printk("bw (%u %u), tx_rate: %u\n", curr_bw, tx_bw, tx_rate);
	panic_printk("1. high_rate_txpwr_A (%u %u), high_rate_txpwr_B (%u %u)\n", 
		curr_high_rate_txpwr[RF_PATH_A], new_high_rate_txpwr[RF_PATH_A], 
		curr_high_rate_txpwr[RF_PATH_B], new_high_rate_txpwr[RF_PATH_B]);
	panic_printk("2. txpwr_A (%u %u), txpwr_B (%u %u)\n", 
		curr_txpwr[RF_PATH_A], new_txpwr[RF_PATH_A], 
		curr_txpwr[RF_PATH_B], new_txpwr[RF_PATH_B]);
	if (!priv->pshare->rf_ft_var.disable_txpwrlmt) {
		panic_printk("3. pwrlmt (%u %u), tgpwr_AB (%u %u)\n", 
			curr_pwrlmt, new_pwrlmt, tg_pwr[RF_PATH_A], tg_pwr[RF_PATH_B]);
		panic_printk("4. max_tpi_A (%u %u), max_tpi_B (%u %u)\n", 
			curr_max_tpi[RF_PATH_A], new_max_tpi[RF_PATH_A], curr_max_tpi[RF_PATH_B], new_max_tpi[RF_PATH_B]);
	}
	panic_printk("5. pwr_offset: %d dbm\n\n", pwr_offset);
#endif

	return pwr_offset;
}

static void init_txpwr_offset_AC_shrink_bw(struct rtl8192cd_priv *priv)
{
	int i;

	//panic_printk("Init txpwr_offset_AC_shrink_bw:\n");

	switch (priv->pshare->CurrentChannelBW)
	{
	case 2:
		for (i = 0; i < 3; i++) {
			priv->pshare->rf_ft_var.txpwr_offset_AC_80_40[i] = calculate_txpwr_offset_AC(priv, VHT_RATE_ID + i, 1);
			priv->pshare->rf_ft_var.txpwr_offset_AC_80_20[i] = calculate_txpwr_offset_AC(priv, VHT_RATE_ID + i, 0);
		}

		//panic_printk("    80=>40:");
		//for (i = 0; i < 3; i++)
		//	panic_printk(" %d", priv->pshare->rf_ft_var.txpwr_offset_AC_80_40[i]);
		//panic_printk("\n");

		//panic_printk("    80=>20:");
		//for (i = 0; i < 3; i++)
		//	panic_printk(" %d", priv->pshare->rf_ft_var.txpwr_offset_AC_80_20[i]);
		//panic_printk("\n");
		
		break;
	case 1:
		for (i = 0; i < 3; i++) {
			priv->pshare->rf_ft_var.txpwr_offset_AC_40_20[i] = calculate_txpwr_offset_AC(priv, VHT_RATE_ID + i, 0);
		}

		//panic_printk("    40=>20:");
		//for (i = 0; i < 3; i++)
		//	panic_printk(" %d", priv->pshare->rf_ft_var.txpwr_offset_AC_40_20[i]);
		//panic_printk("\n");
...
#endif
#ifdef CHECK_HANGUP
	if (priv->hci_type == RTL_HCI_USB || (priv->hci_type == RTL_HCI_PCIE && !is_reset))
#endif
	{
...
#ifdef DFS_METEOROLOGY_REFINE
				priv->limit_bw_20M = 0;
				if(priv->pmib->dot11RFEntry.dot11channel==116 && priv->pmib->dot11nConfigEntry.dot11nUse40M != HT_CHANNEL_WIDTH_20) {
...
#ifdef CHECK_HANGUP
	if (priv->hci_type == RTL_HCI_PCIE && priv->reset_hangup)
		priv->reset_hangup = 0;
#endif
...
//	if (IS_VXD_INTERFACE(priv) && !GET_ROOT_PRIV(priv)->pmib->miscEntry.func_off) 
...
#ifdef CHECK_HANGUP
				if (priv->hci_type == RTL_HCI_USB || (priv->hci_type == RTL_HCI_PCIE && (!is_reset || JOIN_RES == STATE_Sta_No_Bss ||
						JOIN_RES == STATE_Sta_Roaming_Scan || JOIN_RES == 0)))
#endif
				{
#ifdef CHECK_HANGUP
					if (priv->hci_type == RTL_HCI_PCIE && is_reset)
...
#endif
	{
		...
		if (priv->hci_type == RTL_HCI_SDIO || priv->hci_type == RTL_HCI_PCIE) {
#ifdef EN_EFUSE
	...
	if (GET_CHIP_VER(priv) == VERSION_8822B) {
		init_txpwr_offset_AC_shrink_bw(priv);
	}
...
static const struct net_device_ops rtl8192cd_netdev_ops_usb = {
    ...
    (LINUX_VERSION_CODE>=KERNEL_VERSION(3,0,0) && LINUX_VERSION_CODE<KERNEL_VERSION(3,2,0))
        ...
        .ndo_do_ioctl           = rtl8192cd_ioctl,
        .ndo_start_xmit         = rtl8192cd_start_xmit,
#if 1//def CONFIG_NETDEV_MULTI_TX_QUEUE
...
#if 1//def CONFIG_RTL_TRIBAND_SUPPORT
	int hci_type = RTL_HCI_PCIE;
	
#ifdef CONFIG_PCI_HCI
	if (wdev->base_addr == BSP_PCIE0_D_MEM
        #ifdef CONFIG_USE_PCIE_SLOT_1
        || wdev->base_addr == BSP_PCIE1_D_MEM
        #endif
        ) {
		hci_type = RTL_HCI_PCIE;
...
#if defined(SMP_SYNC) //use lock before wlan open will cause false alarm
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
    priv->mesh_mp_hdr_lock_owner = -1;
#endif
    priv->hash_list_lock_owner   = -1;
...
#if defined(CONFIG_ZYXEL_KERNEL_EVENT_LOG)
    zyxel_netlink_init();
#endif // CONFIG_ZYXEL_KERNEL_EVENT_LOG
