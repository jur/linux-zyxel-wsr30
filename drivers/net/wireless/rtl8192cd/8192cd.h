...
#ifdef CONFIG_WLAN_HAL
	//PVOID   phw_desc1;
	u1Byte  hal_hw_desc[48];// 48 is SIZE_TXDESC_88XX
...
#endif
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
    struct tx_sc_entry	tx_sc_ent_usb[NR_NET80211_UP][TX_SC_ENTRY_NUM];
    int					tx_sc_replace_idx_usb[NR_NET80211_UP];
#endif
    int					tx_sc_idx_cache;
    int 				protection;
...
#endif
#if 1//def CONFIG_XMITBUF_TXAGG_ADV
...
#endif
	char		txpwr_offset_AC_80_40[3];		// power offset of shrink band width from 80M to 40M, VHT NSS1 MCS0~MCS2, dbm
	char		txpwr_offset_AC_80_20[3];
	char		txpwr_offset_AC_40_20[3];
...
#endif
#if 1//defined(CONFIG_TCP_ACK_TXAGG) || defined(CONFIG_XMITBUF_TXAGG_ADV)
...
#if 1//defined(CONFIG_RTL_TRIBAND_SUPPORT)//TRIBAND_OPS
struct hci_ops_s {
...
/*
	u8 (*read8)(struct rtl8192cd_priv *priv, u32 addr);
	u16 (*read16)(struct rtl8192cd_priv *priv, u32 addr);
	u32 (*read32)(struct rtl8192cd_priv *priv, u32 addr);
	int (*write8)(struct rtl8192cd_priv *priv, u32 addr, u8 val);
	int (*write16)(struct rtl8192cd_priv *priv, u32 addr, u16 val);
	int (*write32)(struct rtl8192cd_priv *priv, u32 addr, u32 val);
*/
};
...
typedef struct rtl8192cd_priv {
    ...
    int	                mesh_mp_hdr_lock_owner;
    unsigned char       mesh_mp_hdr_lock_func[50];
    ...
    int                 hash_list_lock_owner;
	int                 asoc_list_lock_owner;
	int	                auth_list_lock_owner;
	int	                sleep_list_lock_owner;
	int	                wakeup_list_lock_owner;

    unsigned char       hash_list_lock_func[50];
	unsigned char       asoc_list_lock_func[50];
	unsigned char       auth_list_lock_func[50];
	unsigned char       sleep_list_lock_func[50];
	unsigned char       wakeup_list_lock_func[50];
#endif
#endif
#ifdef WIFI_SIMPLE_CONFIG
...
#if defined(CONFIG_RTL_TRIBAND_SUPPORT) && defined(CONFIG_GEMTEK_BOARD)
#define MESH_UP_WITHOUT_DEAMON
#define MESH_DEFAULT_ROUTE
#endif
...
typedef struct _dfs_event_k {
    int orig_ch;
    int new_ch;
    int bw;
}dfs_event_t, *dfs_event_p;
...
typedef struct _client_event_s {
    unsigned char cli_mac_addr[6];
	unsigned char cli_conn_if[16];
	//  unsigned int interface_id; /* For extension. */
	int behavior;
}cli_event_t, *cli_event_p;

typedef union comm_data_t {
    dfs_event_t dfs_data;
    cli_event_t cil_data;
}comm_data_u;

typedef struct _kern_event_s {
    unsigned char msgtype;
    comm_data_u comm_data;
    int pid;
}kern_event_t, *kern_event_p;
