...
phydm_set_bb_txagc_offset(
	IN		PVOID		pDM_VOID,
	IN		s1Byte		power_offset, /*@(unit: dB)*/
	IN		u1Byte		add_half_db /*@(+0.5 dB)*/
)
{
	PDM_ODM_T	pDM_Odm = (PDM_ODM_T)pDM_VOID;
	s1Byte		power_idx = power_offset * 2;
	...
	if (pDM_Odm->SupportICType & ODM_IC_11AC_SERIES) {
		if (power_offset > -16 && power_offset < 15) {
			...
			ODM_SetBBReg(pDM_Odm, 0x8b4, 0x3f, power_idx);
			set_success = true;
		} else {
	...
	if (pDM_Odm->SupportICType & ODM_IC_11N_SERIES) {
		if (power_offset > -8 || power_offset < 7) {
			...
			/*r_txagc_offset_a*/
			ODM_SetBBReg(pDM_Odm, 0x80c, 0x1f00, power_idx);
			/*r_txagc_offset_b*/
			ODM_SetBBReg(pDM_Odm, 0x80c, 0x3e000, power_idx);
			set_success = true;
		} else {
			pr_debug("[Warning] TX AGC Offset Setting error!");
		}
