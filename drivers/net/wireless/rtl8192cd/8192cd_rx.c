            ...
            if (OPMODE & WIFI_AP_STATE)
            {
                ...
                )
				{
	                ...
	                if(priv->pshare->rf_ft_var.a4_enable == 2 && fdb_find_for_driver(GET_BR_PORT(priv->dev)->br, da) == NULL) {
	                    ...
	                    if (pnewskb) {
	                        ...
	                        if ( 0
								#ifdef GBWC
								|| (priv->hci_type == RTL_HCI_PCIE && GBWC_forward_check(priv, pnewskb, pstat))
								#endif
								#ifdef SBWC
                ...
                else
                {
					...
					//for block_relay is not zero --end-->
                        ...
                        if ( 0 
							#ifdef GBWC
							|| (priv->hci_type == RTL_HCI_PCIE && GBWC_forward_check(priv, pfrinfo->pskb, pstat))
							#endif
							#ifdef SBWC
...
//			printk("%s(%d), reuse(0x%x)\n", __FUNCTION__, __LINE__, (unsigned int)priv->ext_stats.rx_reuse);
	...
	if (IS_SUPPORT_HW_FILL_MACID(priv)) {
			...
			if (new_skb == NULL) {
				DEBUG_WARN("out of skb_buff\n");
                priv->ext_stats.rx_reuse++;
#if 1 // ZYXEL workaround, remove massive debug message.
                // Remove debug message and using DEBUG_WARN instead printk.
                DEBUG_WARN("%s(%d), reuse(0x%x)\n", __FUNCTION__, __LINE__, (unsigned int)priv->ext_stats.rx_reuse);
#else
				printk("%s(%d), reuse(0x%x)\n", __FUNCTION__, __LINE__, (unsigned int)priv->ext_stats.rx_reuse);
#endif
...
static struct rx_frinfo *defrag_frame_main(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
	...
	if (!GetPrivacy(pframe) && privacy != _NO_PRIVACY_)
        ...
        if (!GetPrivacy(pframe) && privacy != _NO_PRIVACY_)
        {
				...
				struct wlan_llc_t) + 3);
				if (amsdu || proto != __constant_ntohs(0x888e)) {
                priv->ext_stats.rx_data_drops++;
...
#ifdef CONFIG_PCI_HCI
/*static*/ void process_APSD_dz_queue_pci(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned short tid)
{
...
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
		if (priv->hci_type == RTL_HCI_USB || priv->hci_type == RTL_HCI_SDIO) {
			process_APSD_dz_queue_usb(priv, pstat, pfrinfo->tid);
		}
#endif
#else //TRIBAND_OPS
	process_APSD_dz_queue(priv, pstat, pfrinfo->tid);
