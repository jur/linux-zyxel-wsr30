...
#endif								
		update_AC_STA_RAMask(priv);
		panic_printk("Swith BW to %d MHz\n", (priv->pmib->dot11nConfigEntry.dot11nUse40M ? (priv->pmib->dot11nConfigEntry.dot11nUse40M*40) : 20));
	} else if(priv->limit_bw_20M ==1) {
		priv->limit_bw_20M =2;
		mod_timer(&priv->ch_avail_chk_timer, jiffies + (CH_AVAIL_CHK_TO_CE-CH_AVAIL_CHK_TO));
		panic_printk("Transmitter is enabled! BW 20MHz temporary!\n");
	} else			
#endif
...
unsigned int DFS_SelectChannel(struct rtl8192cd_priv *priv)
{
	...
	priv->pmib->dot11nConfigEntry.dot11nUse40M = priv->pshare->rf_ft_var.dfs_original_bw;
