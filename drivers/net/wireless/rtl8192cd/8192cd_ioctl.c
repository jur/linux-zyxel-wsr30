...
static struct iwpriv_arg mib_table[] = {
	...
	{"mesh_asoc_rssi_th",	INT_T,	_OFFSET(dot11StationConfigEntry.meshAssociateRSSIThreshold), _SIZE(dot11StationConfigEntry.meshAssociateRSSIThreshold), 0},	
	{"rm_mesh_rssi_th",	INT_T,	_OFFSET(dot11StationConfigEntry.RmMeshRSSIThreshold), _SIZE(dot11StationConfigEntry.RmMeshRSSIThreshold), 0},
	{"rm_mesh_period",	INT_T,	_OFFSET(dot11StationConfigEntry.RmMeshCheckPeriod), _SIZE(dot11StationConfigEntry.RmMeshCheckPeriod), 1},	
...
#elif 0//defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI) //CONFIG_USB_HCI/CONFIG_SDIO_HCI set value in set_mib_default
...
#else
	{"ss_delay",    INT_T,      _OFFSET(miscEntry.ss_delay), _SIZE(miscEntry.ss_delay), 0},
#endif
#endif
...
//	{"antSw_dump",			RFFT_T, _OFFSET_RFFT(antSw_dump), _SIZE_RFFT(antSw_dump), 0},
    ...
    {"enable",		BYTE_T, _OFFSET(sr_profile.enable),	_SIZE(sr_profile.enable),	0},
#endif
...
#if 1 // ZYXEL debug, according to RTK suggestion, to prevent the DUT reboot caused by watchdog timeout when using iwpriv wlan0 reg_dump all command.
		printk("%s[%d]: Prohibit watch dog biting, kick off watchdog!!\n", __func__, __LINE__);
		watchdog_kick();
#endif
	}
}
...
#endif //TRIBAND_MESH_SINGLE_IFACE
