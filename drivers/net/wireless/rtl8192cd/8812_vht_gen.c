...
void construct_vht_ie(struct rtl8192cd_priv *priv, unsigned char channel_center)
{
	...
	int CurrentChannelBW = priv->pshare->CurrentChannelBW;
	int use40M = priv->pmib->dot11nConfigEntry.dot11nUse40M;
	int is40m = priv->pshare->is_40m_bw;
	if(GET_ROOT(priv)->limit_bw_20M) {
		priv->pmib->dot11nConfigEntry.dot11nUse40M =0;
		priv->pshare->CurrentChannelBW = 0;
		priv->pshare->is_40m_bw = 0;
		channel_center = priv->pmib->dot11RFEntry.dot11channel;
	}
#endif
...
#ifdef DFS_METEOROLOGY_REFINE
	if(GET_ROOT(priv)->limit_bw_20M) {
		priv->pshare->CurrentChannelBW = CurrentChannelBW;
		priv->pmib->dot11nConfigEntry.dot11nUse40M = use40M;
		priv->pshare->is_40m_bw = is40m;
	}
#endif
}
