...
{
	struct rtl8192cd_priv *priv = container_of(&(*pmib), struct rtl8192cd_priv, pmib);
...
//			phw->AC_seq[tid] = (phw->AC_seq[tid] + 1) & 0xfff;
...
#endif
	{
		...
		if (priv->hci_type == RTL_HCI_PCIE) {
			...
			seq = atomic_read(&phw->seq);
			SetSeqNum(pframe, seq);
			atomic_set(&phw->seq,  (seq + 1) & 0xfff);
#else		
			SetSeqNum(pframe, phw->seq);
			phw->seq = (phw->seq + 1) & 0xfff;
#endif
...
//eric-7260
			...
			if (GET_CHIP_VER(priv) == VERSION_8822B) {
				// Increase tx power when RA shrink band width
				unsigned char curr_bw = priv->pshare->CurrentChannelBW;
				unsigned char curr_tx_bw = ODMPTR->DM_RA_Table.link_tx_bw[REMAP_AID(txcfg->pstat)];
				unsigned int tx_rate = txcfg->pstat->current_tx_rate;
				char pwr_offset;

				if (tx_rate >= _NSS1_MCS0_RATE_ && tx_rate < _NSS1_MCS3_RATE_) {
					if (curr_bw == 2 && curr_tx_bw == 1)
						pwr_offset = priv->pshare->rf_ft_var.txpwr_offset_AC_80_40[tx_rate - VHT_RATE_ID];
					else if (curr_bw == 2 && curr_tx_bw == 0)
						pwr_offset = priv->pshare->rf_ft_var.txpwr_offset_AC_80_20[tx_rate - VHT_RATE_ID];
					else if (curr_bw == 1 && curr_tx_bw == 0)
						pwr_offset = priv->pshare->rf_ft_var.txpwr_offset_AC_40_20[tx_rate - VHT_RATE_ID];

					phydm_set_bb_txagc_offset(ODMPTR, pwr_offset, 0);
					pdesc_data->TXPowerOffset = 6;		// decide by reg
				}
#if 0	// debug print
				if (curr_bw != curr_tx_bw) {
					panic_printk("shrink tx_bw: tx_rate %u, curr_bw %u, rf_bw %u, pwr_offset %u\n", 
						tx_rate, curr_tx_bw, curr_bw, pwr_offset);
				} else if (curr_bw == curr_tx_bw) {
					panic_printk("recover tx_bw: tx_rate %u, curr_bw %u, rf_bw %u, pwr_offset %u\n", 
						tx_rate, curr_tx_bw, curr_bw, pwr_offset);
				}
#endif
			}
...
#endif
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
		else if (run && (priv->pshare->rf_ft_var.low_retry_rssi_thd && txcfg->pstat->rssi
				&& (txcfg->pstat->rssi < priv->pshare->rf_ft_var.low_retry_rssi_thd))) {
			if ((txcfg->pstat->current_tx_rate <= _9M_RATE_) ||
				((_MCS0_RATE_ == txcfg->pstat->current_tx_rate) || (_MCS1_RATE_ == txcfg->pstat->current_tx_rate))) {
				pdesc->Dword5 |= set_desc(TX_RtyLmtEn);
...
#if defined(CONFIG_RTL_TRIBAND_SUPPORT) && defined(CONFIG_RTK_MESH)
if (priv->hci_type == RTL_HCI_USB && txcfg->is_11s) {
    txcfg->fr_type = _SKB_FRAME_TYPE_;
    txcfg->pframe = skb;
    txcfg->pstat = pstat;
...
//	int priority;
	...
	if (!priv->pmib->dot11OperationEntry.disable_txsc && pstat) {
		...
		if ((k = get_tx_sc_index_usb(priv, pstat, skb->data, pktpri)) >= 0)
			ptxsc_entry = &pstat->tx_sc_ent_usb[pktpri][k];
	}
...
#if defined(CONFIG_PCI_HCI)
	if (priv->hci_type == RTL_HCI_PCIE) {
		ret = __rtl8192cd_start_xmit_pci(skb, dev, tx_flag);
	}
#endif
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	if (priv->hci_type == RTL_HCI_USB || priv->hci_type == RTL_HCI_SDIO) {
		ret = __rtl8192cd_start_xmit_usb(skb, dev, tx_flag);
	}
#endif
	return ret;
}
...
#if defined(CONFIG_PCI_HCI)
	if (priv->hci_type == RTL_HCI_PCIE) {
		ret = rtl8192cd_firetx_pci(priv, txcfg);
	}
#endif
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	if (priv->hci_type == RTL_HCI_USB || priv->hci_type == RTL_HCI_SDIO) {
		ret = rtl8192cd_firetx_usb(priv, txcfg);
	}
#endif
	return ret;
}
