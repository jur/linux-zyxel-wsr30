...
#if defined(CONFIG_RTL_TRIBAND_SUPPORT) && defined(CONFIG_RTK_MESH) //TRIBAND_MESH
extern unsigned int get_skb_priority3(struct rtl8192cd_priv *priv, struct sk_buff *skb, int is_11s, struct stat_info *pstat);
...
int get_tx_sc_index_usb(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *hdr, unsigned char pktpri)
{
	...
	ptxsc_entry = pstat->tx_sc_ent_usb[pktpri];
...
int get_tx_sc_free_entry_usb(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *hdr, unsigned char pktpri)
{
	...
	// no free entry
	i = pstat->tx_sc_replace_idx_usb[pktpri];
	pstat->tx_sc_replace_idx_usb[pktpri] = (i+1) % TX_SC_ENTRY_NUM;
	return i;
}
#endif // TX_SHORTCUT
...
//#if	defined(BATMAN_ENABLE)
		/* fix kernel panic on batman module */
//		if(pstat < 0x80000000) {
			//WARN_ON(1);
//			return	FALSE;
//		}
