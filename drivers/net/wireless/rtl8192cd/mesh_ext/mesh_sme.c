...
#ifdef MESH_UP_WITHOUT_DEAMON
    priv->pmib->dot1180211sInfo.mesh_portal_enable = 1;
#endif
    return 0;
}

#if		defined(BATMAN_ENABLE)
/* TODO: add rcu lock to protect hlist */
HLIST_HEAD(mesh_head);
EXPORT_SYMBOL(mesh_head);
...
const int	MCS_DATA_RATEFloat_[2][2][/*16*/24] =
...
const u2Byte	VHT_MCS_DATA_RATE_[3][2][30] =				// 20/40/80,   ShortGI,    MCS Rate
...
DEFINE_RATELIMIT_STATE(batman_ratelimit_state, 10*HZ, 1);

/* return -ENOENT if failed
 *        current tx rate if successfully
 */
int	batadv_get_wifi_link_rate
(struct net_device *netdev, 
 u8 *orig)
{
	struct stat_info	*pstat;
	unsigned int		tx_rate=-ENOENT, tmp=0, band=0;
	struct rtl8192cd_priv	*priv=GET_DEV_PRIV(netdev);
	//struct dev_priv   *dp=(struct dev_priv *)NETDRV_PRIV(netdev);
	struct list_head	*phead, *plist;

	if(!priv) {
		printk(KERN_ERR "[BATMAN] ERROR! wrong netdev\n");
		goto	RETN;
	}
#if	1//just for debug
...
#if	0//method-1
	pstat=get_stainfo(/*dp*/priv, orig);
	/*workaround*/
	WARN_ON(pstat == NULL);
	if(pstat == NULL) {
		printk(KERN_ERR "[BATMAN] pstat:%p pstat->current_tx_rate:%p\n",	
			pstat, &pstat->current_tx_rate);
		goto	RETN;
	}
	tmp=pstat->current_tx_rate;
#endif
#if	0//method-2
	tmp=priv->pshare->current_tx_rate;
#endif
#if	0//method-3
	phead=&priv->asoc_list;
	if(!phead) {
		if(__ratelimit(&batman_ratelimit_state))
			printk(KERN_ERR "[BATMAN] Err!!! assoc list is null!!!\n");
		goto	RETN;
	}
	if(!(priv->drv_state&DRV_STATE_OPEN) || list_empty(phead)) {
		if(__ratelimit(&batman_ratelimit_state))
			printk(KERN_INFO "[BATMAN] priv->drv_state isn't DRV_STATE_OPEN or list empty!\n");
		goto	RETN;
	}
	plist=phead->next;
	if(!plist) {
		if(__ratelimit(&batman_ratelimit_state))
			printk(KERN_INFO "[BATMAN] Err! plist is empty!\n");
		goto	RETN;
	}
	while(plist != phead) {
		...
		if(pstat) {
			int	compare=1;
			tmp=pstat->current_tx_rate;
			//printk(KERN_INFO "[BATMAN] mac: %x %x %x\n", pstat->hwaddr[3], pstat->hwaddr[4], pstat->hwaddr[5]);

			/* find out stat_info which corresponds to orig */
			if(memcmp(orig, pstat->hwaddr, ETH_ALEN) != 0) {
				continue;
			}
			if(tmp >= VHT_RATE_ID) {
				tx_rate=query_vht_rate(pstat);
			} else if(is_MCS_rate(tmp)) {
				tx_rate=MCS_DATA_RATEFloat_[pstat->ht_current_tx_info&BIT(0)?1:0][pstat->ht_current_tx_info&BIT(1)?1:0][tmp&0xf];
			} else {
				tx_rate=tmp/2;
			}
			goto	RETN;
			...
			WARN_ON(1);
			if(__ratelimit(&batman_ratelimit_state))
				printk(KERN_ERR "[BATMAN] Error!!! stat_info is null!!!\n");
			goto	RETN;
		}
	}
	printk(KERN_INFO "[BATMAN] error no entry!\n");
#endif
#if	1//method-4
	phead=&priv->mesh_mp_hdr;
	if(!netif_running(priv->mesh_dev) || list_empty(phead)) {
		if(__ratelimit(&batman_ratelimit_state))
			printk(KERN_ERR "[BATMAN] Err! mesh dev isn't running or list empty!\n");
		goto    RETN;
	}
	plist=phead->next;
	while(plist != phead) {
		pstat=list_entry(plist, struct stat_info, mesh_mp_ptr);
		if(memcmp(pstat->hwaddr, orig, ETH_ALEN)) {
			/*printk(KERN_INFO "[B@TMAN] mac %x %x %x %x %x %x VS %x %x %x %x %x %x\n", */
			//printk(KERN_INFO "[B@TMAN] mac %x %x %x VS. %x %x %x\n", 
		...
		if(pstat->current_tx_rate >= VHT_RATE_ID) {
			tx_rate=query_vht_rate(pstat);
			//printk(KERN_INFO "[B@TMAN] vht rate: %d\n", tx_rate);
		} else
#endif	//RTK_AC_SUPPORT
		if(is_MCS_rate(pstat->current_tx_rate)) {
			tx_rate=MCS_DATA_RATEFloat_[pstat->ht_current_tx_info&BIT(0) ? 1:0][pstat->ht_current_tx_info&BIT(1) ? 1:0][pstat->current_tx_rate - HT_RATE_ID];
		} else {
...
#if	1//method-1
		if(pstat)
			/*tx_rate*/test=query_vht_rate(pstat);
#endif
		tx_rate=VHT_MCS_DATA_RATE_[priv->pshare->is_40m_bw][(priv->pshare->ht_current_tx_info&BIT(1))?1:0][(priv->pshare->current_tx_rate - VHT_RATE_ID)];
		tx_rate = tx_rate >> 1;
	} else if(is_MCS_rate(tmp)) {
#if	1//method-1
		if(pstat)
			/*tx_rate*/test=MCS_DATA_RATEFloat_[pstat->ht_current_tx_info&BIT(0)?1:0][pstat->ht_current_tx_info&BIT(1)?1:0][tmp&0xf];
#endif	
		tx_rate=MCS_DATA_RATEFloat_[(priv->pshare->ht_current_tx_info&BIT(0))?1:0][(priv->pshare->ht_current_tx_info&BIT(1))?1:0][(priv->pshare->current_tx_rate - HT_RATE_ID)];
	} else {
		tx_rate=tmp/2;
	}
#endif
RETN:
#if	0//for debug
	//printk(KERN_INFO "[BATMAN] dp:%p tx:%d\n", dp, tx_rate);
	//printk(KERN_INFO "[BATMAN] pstat:%p ht_current_tx:%x curr_rate:%d tx:%d\n", 
	//  pstat, pstat->ht_current_tx_info, pstat->current_tx_rate&0xf, tx_rate);
	//printk(KERN_INFO "[BATMAN] pstat:%p pstat->current_tx_rate:%p tmp:%d test:%d\n", 
	//	pstat, &pstat->current_tx_rate, tmp, test);
	printk(KERN_INFO "[BATMAN] (band%d)(%x:%x:%x) %d\n", 
		band, orig[3], orig[4], orig[5], tx_rate);
#endif
	return	tx_rate;
}
EXPORT_SYMBOL(batadv_get_wifi_link_rate);
#endif	//BATMAN_ENABLE
...
static void updateMeshMetric(DRV_PRIV *priv, struct stat_info *pstat, struct rx_frinfo *pfrinfo)
{
    ...
    */
...
#if		defined(BATMAN_ENABLE)
	struct wifi_mesh_node	*cpy, *new, *tmp;
	u8	self[ETH_ALEN]={0}, 
		found=0, 
		band=0;
	u32	ori_rssi=0, 
	...
	memcpy(self, GET_MY_HWADDR, ETH_ALEN);
	/*band*/
	if(priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_2G)
		band=2;
	else if(priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_5G)
	...
	hlist_for_each_entry/*_rcu*/(tmp, &mesh_head, list) {
		if(!memcmp(tmp->orig, pfrinfo->sa, ETH_ALEN) && !memcmp(tmp->addr, self, ETH_ALEN) && (band==tmp->band)) {
			new=tmp;
			found=1;
			ori_rssi=new->rssi_metric;
			ori_cu=new->cu_metric;
			ori_noise=new->noise_metric;
			max_rssi=new->rssi_metric+2;
			min_rssi=new->rssi_metric-2;
			break;
		}
	}
	if(!found) {
		new=kzalloc(sizeof(struct wifi_mesh_node), GFP_KERNEL);
		INIT_HLIST_NODE(&new->list);
		/* orig & self-iface */
		memcpy(new->orig, pfrinfo->sa, ETH_ALEN);
		memcpy(new->addr, self, ETH_ALEN);
		new->band=band;
		hlist_add_head_rcu(new, &mesh_head);

		/* metric */
		new->rssi_metric=pfrinfo->rssi;
		new->cu_metric=priv->envinfo.cu_metric;
		new->noise_metric=priv->envinfo.noise_metric;
	} else if(found) {
#if	1
		if((ori_cu==priv->envinfo.cu_metric) && (ori_noise==priv->envinfo.noise_metric)) {
			if(new->dirty == -1) {
				if(ori_rssi != pfrinfo->rssi) {
					new->dirty=1;
					new->rssi_metric=pfrinfo->rssi;
				} else {
					//new->dirty=-1;
				...
				if(ori_rssi != pfrinfo->rssi) {
					if((min_rssi<=pfrinfo->rssi) && (pfrinfo->rssi<=max_rssi)) {
						new->dirty=-1;
					} else {
						new->dirty=1;
					}
...
#else
		if((ori_rssi==pfrinfo->rssi) && (ori_cu==priv->envinfo.cu_metric) && (ori_noise==priv->envinfo.noise_metric)) {
			new->dirty=1;
		} else {
			printk(KERN_INFO "[BATMAN] band(%d) !!!dirty\n", new->band);
			/*metric*/
			new->rssi_metric=pfrinfo->rssi;
			new->cu_metric=priv->envinfo.cu_metric;
			new->noise_metric=priv->envinfo.noise_metric;
...
#if	0//for debug
		if(new->dirty == 1)
			printk(KERN_ERR "[BATMAN] band(%d)  dirty(%d) ", new->band, new->dirty);
		else if(new->dirty == -1)
			 printk(KERN_ERR "[BATMAN] band(%d) fresh but valid(%d) ", new->band, new->dirty);
		/*printk(KERN_INFO "%d-%d-%d %x:%x:%x:%x:%x:%x   %x:%x:%x:%x:%x:%x\n", 
			new->rssi_metric, new->cu_metric, new->noise_metric, 
			new->orig[0], new->orig[1], new->orig[2], new->orig[3], new->orig[4], new->orig[5], 
			new->addr[0], new->addr[1], new->addr[2], new->addr[3], new->addr[4], new->addr[5]);*/
#endif
	new->timeval=jiffies+600*HZ;    //refresh timeout
#endif	//BATMAN_ENABLE
...
int start_MeshPeerLink(DRV_PRIV *priv, struct rx_frinfo *pfrinfo, struct stat_info *pstat, UINT16 cap)
{
    ...
    if (NULL == pstat)
    {
		...
		if (pfrinfo->rssi < priv->pmib->dot11StationConfigEntry.meshAssociateRSSIThreshold) {
			return SUCCESS;
        }			
...
static void mesh_auto_channel_check(DRV_PRIV *priv, struct rx_frinfo *pfrinfo, UINT32 precedence,int force_apply)
{
	...
	if (!strcmp(GET_ROOT(priv)->dev->name, "wlan2"))
		return;
    // OFDM parameter set
