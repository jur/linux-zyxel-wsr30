...
#ifdef CONFIG_RTL_ROMEPERF_1074K
		...
		else
		{
			...
			if (!memcmp(cmd_addr, "dump", 4))
			{
				...
				rtlglue_printf("Vlan info:\n");
				{
                    ...
                    uint16 index = vid;
					int flag = rtl8651_findAsicVlanIndexByVid(&index);

					if(flag == FAILED )
						return len;
					
					if ( rtl8651_getAsicVlan( index, &vlan ) == FAILED )
					#else
				...
				(!memcmp(cmd_addr, "ADD", 3))	)
			{
				...
				untaggedPortMask= simple_strtol(tokptr, NULL, 16);
				
				rtl865x_addVlan(vid);
				//rtl865x_addVlanPortMember(vid & 4095,portMask & 0x13F);
				//rtl865x_setVlanPortTag(vid,taggedPortMask,1);
				rtl865x_addVlanPortMember2(vid & 4095,portMask, untaggedPortMask);
...
#ifdef CONFIG_RTL_ROMEPERF_1074K
{
    ...
    unsigned long int cp3AvgValue[4];

    curValue = (unsigned int*)& cp3Value[0];
    cp3AvgValue[0] = (curValue[0]*(0xffffffff/cp3Count[0])) + (curValue[1]/cp3Count[0]);
    
    curValue = (unsigned int*)& cp3Value[1];
    cp3AvgValue[1] = (curValue[0]*(0xffffffff/cp3Count[1])) + (curValue[1]/cp3Count[1]);
    
    curValue = (unsigned int*)& cp3Value[2];
    cp3AvgValue[2] = (curValue[0]*(0xffffffff/cp3Count[2])) + (curValue[1]/cp3Count[2]);

    curValue = (unsigned int*)& cp3Value[3];
    cp3AvgValue[3] = (curValue[0]*(0xffffffff/cp3Count[3])) + (curValue[1]/cp3Count[3]);
	
	seq_printf(s,"    cp3Value0:	%llu\n",cp3Value[0]);
	seq_printf(s,"    cp3Value1:	%llu\n",cp3Value[1]);
	seq_printf(s,"    cp3Value2:	%llu\n",cp3Value[2]);
	seq_printf(s,"    cp3Value3:	%llu\n",cp3Value[3]);
	
	seq_printf(s,"    cp3Count0:	%lu\n",cp3Count[0]);
	seq_printf(s,"    cp3Count1:	%lu\n",cp3Count[1]);
	seq_printf(s,"    cp3Count2:	%lu\n",cp3Count[2]);
	seq_printf(s,"    cp3Count3:	%lu\n",cp3Count[3]);
	
	seq_printf(s,"    cp3Avg0:	%lu\n",cp3AvgValue[0]);
	seq_printf(s,"    cp3Avg1:	%lu\n",cp3AvgValue[1]);
	seq_printf(s,"    cp3Avg2:	%lu\n",cp3AvgValue[2]);
	seq_printf(s,"    cp3Avg3:	%lu\n",cp3AvgValue[3]);
}
#endif /* CONFIG_RTL_ROMEPERF_1074K */
...
/* input: PHY0/PHY1/.../PHY5
 * return: 0 if successfully, -1(SPEEDN_UNKNOWN) if failed
 *         link up/down (1/0)
 *         full/half duplex (1/0)
 *         10/100/1000/500M: 10/100/1000/500
 */
int		batadv_get_port_status
(struct net_device *netdev, 
 struct port_status *status)
{
	int port=0;
	int retval=SPEED_UNKNOWN;
	u32 regData, regData_PCR, data;
	struct dev_priv *dp;
#if     !defined(WSR30_BOARD)
	int j;
#endif
	int	this_port=0;

	dp=(struct dev_priv *)NETDRV_PRIV(netdev);
	status->link=0;
	status->duplex=0;
	status->speed=0;
	//memset(status, 0x0, sizeof(struct port_status));
#if		defined(WSR30_BOARD)			//WSR30 board
	if(dp->portnum == 1) {				//eth1
		this_port=4;					//PHY4
	} else if(dp->portnum == 5) {		//eth0
		this_port=1;					//PHY1
	} else if(!dp->portnum && !dp->portmask) {	//vlan
		if(strstr(netdev->name, "eth0") != 0)	//eth0.xxx
			this_port=1;
		else if(strstr(netdev->name, "eth1") != 0)	//eth1.xxx
			this_port=4;
		else
			printk(KERN_ERR "[B@TMAN] err! wrong iface: %s\n", netdev->name);
	}
#else									//DEMO board
	for(j=0; j<RTL8651_AGGREGATOR_NUMBER; j++) {
		//printk(KERN_INFO "[%d] portnum(%d) portmask(%x) (%d)\n",
		//	j, dp->portnum, dp->portmask, RTL8651_AGGREGATOR_NUMBER);
		if(dp->portmask & (1<<j)) {
			//dp->portnum++;
			this_port=j;
			//for(port=PHY0; port<CPU; port++) {
				...
				/* link status */
				if(data) {
					status->link=1;	//link up
					retval=0;
					//printk(KERN_INFO "[%d] Link-up ", port);
				} else {
					status->link=0;	//link down
				}
				if(status->link == 1) {
					/* full/half duplex */
					data=regData & PortStatusDuplex;
					if(data) {
						status->duplex=1;	//full duplex
					} else {
						status->duplex=0;	//half duplex
...
#endif
	/*printk(KERN_INFO "[*] mac addr %x %x %x %x %x %x\n", 
		dp->dev->dev_addr[0], dp->dev->dev_addr[1], dp->dev->dev_addr[2], 
		dp->dev->dev_addr[3], dp->dev->dev_addr[4], dp->dev->dev_addr[5]);
	printk(KERN_INFO "[*] mac addr %x %x %x %x %x %x\n", 
		netdev->dev_addr[0], netdev->dev_addr[1], netdev->dev_addr[2], 
		netdev->dev_addr[3], netdev->dev_addr[4], netdev->dev_addr[5]);*/
	return	retval;
}
EXPORT_SYMBOL(batadv_get_port_status);
#endif	//BATMAN_ENABLE
