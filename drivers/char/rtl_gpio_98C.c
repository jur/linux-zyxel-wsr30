...
#ifdef CONFIG_ZYXEL_GPIO_CONTROL_SUPPORT
#define ZYXEL_LED_CTRL
#else
#define AUTO_CONFIG
#endif
...
void RTL_BSP_GPIO_write(unsigned int gpio_num, unsigned int value)
{
	...
	#define RESET_LED_DIRBASE   PEFGH_DIR	 
	#define RESET_LED_DATABASE  PEFGH_DAT 
#ifndef CONFIG_ZYXEL_GPIO_CONTROL_SUPPORT
	//#define RESET_LED_NO 7+24 /*number of the EFGH*/
#endif	// CONFIG_ZYXEL_GPIO_CONTROL_SUPPORT
	// GPIO G0
	#define AUTOCFG_LED_IOBASE      PEFGH_CNR 	
	#define AUTOCFG_LED_DIRBASE     PEFGH_DIR	 
	#define AUTOCFG_LED_DATABASE    PEFGH_DAT 
...
#ifdef ZYXEL_LED_CTRL
static int led_read_proc(struct seq_file *s, void *v)
{
	char led_flag;
			...
			unsigned long count, void *data)
{
	...
	if (count < 2)
		return -EFAULT;
	if (buffer && !copy_from_user(&led_flag, buffer, 1)) {
		return count;
	}
	return -EFAULT;
}

int led_ctrl_single_open(struct inode *inode, struct file *file)
{
	return(single_open(file, led_read_proc, NULL));
}

static ssize_t led_ctrl_single_write(struct file * file, const char __user * userbuf,
			size_t count, loff_t * off)
{
	return led_write_proc(file, userbuf,count, off);
}

struct file_operations led_ctrl_proc_fops = {
	.open			= led_ctrl_single_open,
	.write			= led_ctrl_single_write,
	.read			= seq_read,
	.llseek			= seq_lseek,
	.release		= single_release,
};
#endif	// ZYXEL_LED_CTRL
...
#ifdef CONFIG_GEMTEK_BOARD
#define SYS_BASE 0xb8000000
#define SYS_PIN_MUX_SEL4 (SYS_BASE +0x10c)
    printk("Enable I2C\n");
    printk("before value reg 0x%x\n",REG32(SYS_PIN_MUX_SEL4));
    RTL_W32(RTL_GPIO_MUX4, ((RTL_R32(RTL_GPIO_MUX4) & ~(15 << 3)) | (6 << 3)));
    printk("middle value reg 0x%x\n",REG32(SYS_PIN_MUX_SEL4));
    RTL_W32(RTL_GPIO_MUX4, ((RTL_R32(RTL_GPIO_MUX4) & ~(15 << 17)) | (8 << 17)));
    printk("after value reg 0x%x\n",REG32(SYS_PIN_MUX_SEL4));
#endif //CONFIG_GEMTEK_BOARD
	...
	RTL_W32(AUTOCFG_PIN_DIRBASE, (RTL_R32(AUTOCFG_PIN_DIRBASE) & (~(1 << AUTOCFG_BTN_PIN))));
#endif
	proc_create_data("led_ctrl", 0, &proc_root,
			 &led_ctrl_proc_fops, NULL);
#endif	// ZYXEL_LED_CTRL
