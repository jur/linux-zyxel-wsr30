...
netdev_tx_t br_dev_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	skb_pull(skb, ETH_HLEN);
...
#if	defined(BATMAN_MESH)
		{
			...
			if(fastmesh_enable) {
			if(_br_fastbat_ops.fastbat_grab_real_ethhdr != NULL) {
				...
				if(pass) {
					...
					if(ethhdr != NULL) {
						...
						if(bat != NULL) {
							_br_fastbat_ops.fastbat_reset_header(skb);
							if(_br_fastbat_ops.fastbat_dispatch(skb, bat, vlan, batman) >= 0) {
								goto	out;
							}
						}
					} else {
						printk(KERN_ERR "[BR B@TMAN] ethhdr is NULL!\n");
					}
				}
			}
			}
			if(show_fastmesh_msg)
				printk(KERN_ERR "[BR B@TMAN] BACK 2 BRIDGE MODULE FROM br_dev_xmit()!\n");
