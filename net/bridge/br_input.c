...
DEFINE_RATELIMIT_STATE(br_fastbat_message_state, 6*HZ, 1);
#endif
...
rx_handler_result_t br_handle_frame(struct sk_buff **pskb)
{
	...
	if(fastmesh_enable) {
		...
		if(_br_fastbat_ops.fastbat_filter_input_packet != NULL) {
			pass=_br_fastbat_ops.fastbat_filter_input_packet(skb, &vlan, &batman);
			if(pass) {
				ethhdr=_br_fastbat_ops.fastbat_grab_real_ethhdr(skb);
				if(ethhdr != NULL) {
					bat=_br_fastbat_ops.fastbat_list_search(ethhdr->h_source, ethhdr->h_dest);
					if(bat != NULL) {
						if(skb->skb_iif != bat->iif)
							_br_fastbat_ops.fastbat_update_arrival_dev(bat, skb->skb_iif);
						if(_br_fastbat_ops.fastbat_dispatch(skb, bat, vlan, batman) >= 0) {
							return	RX_HANDLER_CONSUMED;
						}
					}
				}
	...
	if(show_fastmesh_msg)
		printk(KERN_ERR "[B@TMAN] BACK 2 B@TMAN_ADV MODULE FROM br_handler_frame()!\n");
#endif
