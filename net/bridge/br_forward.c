...
int br_dev_queue_push_xmit(struct sk_buff *skb)
{
	...
	if (nf_bridge_maybe_copy_header(skb) ||
		...
		if(fastmesh_enable) {
			...
			//printk(KERN_ERR "[BR B@TMAN] (xmit) here you go %pM -> %pM!!!\n", ethhdr->h_source, ethhdr->h_dest);
			if(/*(skb->dev!=NULL) &&*/ (_br_fastbat_ops.fastbat_chk_and_add_local_entry!=NULL)) {
				_br_fastbat_ops.fastbat_chk_and_add_local_entry(skb, skb->dev);
			} else
				printk(KERN_ERR "[BR F@STBAT] dev or entry point is null!\n");
		}
#endif
		dev_queue_xmit(skb);
	}
